/*
 * Copyright (c) 2020, 2024, Oracle and/or its affiliates. All rights reserved.
 * This software is dual-licensed to you under the Universal Permissive License (UPL) 1.0 as shown at https://oss.oracle.com/licenses/upl or Apache License 2.0 as shown at http://www.apache.org/licenses/LICENSE-2.0. You may choose either license.
 */

// NOTE: Code generated by OracleSDKGenerator.
// DO NOT EDIT this file manually.


using System.ComponentModel.DataAnnotations;
using System.Runtime.Serialization;

namespace Oci.GenerativeaiagentService.Models
{
  /// <summary>
  /// The asynchronous operation tracked by the work request.
  /// </summary>
  public enum OperationType {
      /// This value is used if a service returns a value for this enum that is not recognized by this version of the SDK.
      [EnumMember(Value = null)]
      UnknownEnumValue,
      [EnumMember(Value = "CREATE_AGENT")]
      CreateAgent,
      [EnumMember(Value = "UPDATE_AGENT")]
      UpdateAgent,
      [EnumMember(Value = "DELETE_AGENT")]
      DeleteAgent,
      [EnumMember(Value = "MOVE_AGENT")]
      MoveAgent,
      [EnumMember(Value = "CREATE_AGENT_ENDPOINT")]
      CreateAgentEndpoint,
      [EnumMember(Value = "UPDATE_AGENT_ENDPOINT")]
      UpdateAgentEndpoint,
      [EnumMember(Value = "DELETE_AGENT_ENDPOINT")]
      DeleteAgentEndpoint,
      [EnumMember(Value = "MOVE_AGENT_ENDPOINT")]
      MoveAgentEndpoint,
      [EnumMember(Value = "CREATE_DATA_SOURCE")]
      CreateDataSource,
      [EnumMember(Value = "UPDATE_DATA_SOURCE")]
      UpdateDataSource,
      [EnumMember(Value = "DELETE_DATA_SOURCE")]
      DeleteDataSource,
      [EnumMember(Value = "CREATE_KNOWLEDGE_BASE")]
      CreateKnowledgeBase,
      [EnumMember(Value = "UPDATE_KNOWLEDGE_BASE")]
      UpdateKnowledgeBase,
      [EnumMember(Value = "DELETE_KNOWLEDGE_BASE")]
      DeleteKnowledgeBase,
      [EnumMember(Value = "MOVE_KNOWLEDGE_BASE")]
      MoveKnowledgeBase,
      [EnumMember(Value = "CREATE_DATA_INGESTION_JOB")]
      CreateDataIngestionJob,
      [EnumMember(Value = "DELETE_DATA_INGESTION_JOB")]
      DeleteDataIngestionJob,
      [EnumMember(Value = "CREATE_TOOL")]
      CreateTool,
      [EnumMember(Value = "UPDATE_TOOL")]
      UpdateTool,
      [EnumMember(Value = "DELETE_TOOL")]
      DeleteTool
  }
}
