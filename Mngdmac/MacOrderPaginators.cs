...
namespace Oci.MngdmacService
{
    ...
    {
        private readonly MacOrderClient client;

        public MacOrderPaginators(MacOrderClient client)
        {
            this.client = client;
        }

        /// <summary>
        /// Creates a new enumerable which will iterate over the responses received from the ListMacOrders operation. This enumerable
        ...
        public IEnumerable<ListMacOrdersResponse> ListMacOrdersResponseEnumerator(ListMacOrdersRequest request, Common.Retry.RetryConfiguration retryConfiguration = null, CancellationToken cancellationToken = default)
        {
            return new Common.Utils.ResponseEnumerable<ListMacOrdersRequest, ListMacOrdersResponse>(
                ...
                request => client.ListMacOrders(request, retryConfiguration, cancellationToken)
            );
        }

        /// <summary>
        /// Creates a new enumerable which will iterate over the MacOrderSummary objects
        /// contained in responses from the ListMacOrders operation. This enumerable will fetch more data from the server as needed.
        /// </summary>
        /// <param name="request">The request object containing the details to send</param>
        /// <param name="retryConfiguration">The configuration for retrying, may be null</param>
        /// <param name="cancellationToken">The cancellation token object</param>
        /// <returns>The enumerator, which supports a simple iteration over a collection of a specified type</returns>
        public IEnumerable<MacOrderSummary> ListMacOrdersRecordEnumerator(ListMacOrdersRequest request, Common.Retry.RetryConfiguration retryConfiguration = null, CancellationToken cancellationToken = default)
        {
            return new Common.Utils.ResponseRecordEnumerable<ListMacOrdersRequest, ListMacOrdersResponse, MacOrderSummary>(
                response => response.OpcNextPage,
                input =>
                {
                    if (!string.IsNullOrEmpty(input))
                ...
                request => client.ListMacOrders(request, retryConfiguration, cancellationToken),
                response => response.MacOrderCollection.Items
            );
        }
