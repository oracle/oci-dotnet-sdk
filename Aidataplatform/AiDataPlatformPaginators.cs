...
namespace Oci.AidataplatformService
{
    ...
    /// to any list operations of AiDataPlatform where multiple pages of data may be fetched.
    ...
    /// </summary>
    public class AiDataPlatformPaginators
    {
        private readonly AiDataPlatformClient client;

        public AiDataPlatformPaginators(AiDataPlatformClient client)
        {
            this.client = client;
        }

        /// <summary>
        /// Creates a new enumerable which will iterate over the responses received from the ListAiDataPlatforms operation. This enumerable
        ...
        public IEnumerable<ListAiDataPlatformsResponse> ListAiDataPlatformsResponseEnumerator(ListAiDataPlatformsRequest request, Common.Retry.RetryConfiguration retryConfiguration = null, CancellationToken cancellationToken = default)
        {
            return new Common.Utils.ResponseEnumerable<ListAiDataPlatformsRequest, ListAiDataPlatformsResponse>(
                ...
                request => client.ListAiDataPlatforms(request, retryConfiguration, cancellationToken)
            );
        }

        /// <summary>
        /// Creates a new enumerable which will iterate over the AiDataPlatformSummary objects
        /// contained in responses from the ListAiDataPlatforms operation. This enumerable will fetch more data from the server as needed.
        /// </summary>
        /// <param name="request">The request object containing the details to send</param>
        /// <param name="retryConfiguration">The configuration for retrying, may be null</param>
        /// <param name="cancellationToken">The cancellation token object</param>
        /// <returns>The enumerator, which supports a simple iteration over a collection of a specified type</returns>
        public IEnumerable<AiDataPlatformSummary> ListAiDataPlatformsRecordEnumerator(ListAiDataPlatformsRequest request, Common.Retry.RetryConfiguration retryConfiguration = null, CancellationToken cancellationToken = default)
        {
            return new Common.Utils.ResponseRecordEnumerable<ListAiDataPlatformsRequest, ListAiDataPlatformsResponse, AiDataPlatformSummary>(
                response => response.OpcNextPage,
                input =>
                {
                    if (!string.IsNullOrEmpty(input))
                ...
                request => client.ListAiDataPlatforms(request, retryConfiguration, cancellationToken),
                response => response.AiDataPlatformCollection.Items
            );
        }
